...
function blackPawn(current_position){return{uid:nextPieceUid(),current_position,img:"Assets/images/pieces/black/pawn.png",piece_name:"BLACK_PAWN"}}
function blackBishop(current_position){return{uid:nextPieceUid(),current_position,img:"Assets/images/pieces/black/bishop.png",piece_name:"BLACK_BISHOP"}}
function blackKnight(current_position){return{uid:nextPieceUid(),current_position,img:"Assets/images/pieces/black/knight.png",piece_name:"BLACK_KNIGHT"}}
function blackKing(current_position){return{uid:nextPieceUid(),move:!1,current_position,img:"Assets/images/pieces/black/king.png",piece_name:"BLACK_KING"}}
function blackQueen(current_position){return{uid:nextPieceUid(),current_position,img:"Assets/images/pieces/black/queen.png",piece_name:"BLACK_QUEEN"}}
function blackRook(current_position){return{uid:nextPieceUid(),move:!1,current_position,img:"Assets/images/pieces/black/rook.png",piece_name:"BLACK_ROOK"}}
function whitePawn(current_position){return{uid:nextPieceUid(),current_position,img:"Assets/images/pieces/white/pawn.png",piece_name:"WHITE_PAWN"}}
function whiteRook(current_position){return{uid:nextPieceUid(),move:!1,current_position,img:"Assets/images/pieces/white/rook.png",piece_name:"WHITE_ROOK"}}
function whiteKnight(current_position){return{uid:nextPieceUid(),current_position,img:"Assets/images/pieces/white/knight.png",piece_name:"WHITE_KNIGHT"}}
function whiteBishop(current_position){return{uid:nextPieceUid(),current_position,img:"Assets/images/pieces/white/bishop.png",piece_name:"WHITE_BISHOP"}}
function whiteQueen(current_position){return{uid:nextPieceUid(),current_position,img:"Assets/images/pieces/white/queen.png",piece_name:"WHITE_QUEEN"}}
function whiteKing(current_position){return{uid:nextPieceUid(),move:!1,current_position,img:"Assets/images/pieces/white/king.png",piece_name:"WHITE_KING"}}
...
function pawnPromotion(color,callback,id){const squareEl=document.getElementById(id);const box=document.createElement("div");box.classList.add("promotionBox");if(color==="black")box.classList.add("promotionBoxUp");const options=[{fn:color==="white"?whiteQueen:blackQueen,src:`Assets/images/pieces/${color}/queen.png`},{fn:color==="white"?whiteKnight:blackKnight,src:`Assets/images/pieces/${color}/knight.png`},{fn:color==="white"?whiteRook:blackRook,src:`Assets/images/pieces/${color}/rook.png`},{fn:color==="white"?whiteBishop:blackBishop,src:`Assets/images/pieces/${color}/bishop.png`},];options.forEach(({fn,src})=>{const img=document.createElement("img");img.src=src;img.onclick=(event)=>{event.stopPropagation();callback(fn,id);box.remove()};box.appendChild(img)});squareEl.appendChild(box)}
...
if(pawnIsPromoted)pawnPromotion(inTurn,callbackPawnPromotion,id);checkForCheck();if(!castle){changeTurn();if(isCheckmate(inTurn)){gameOver=!0;showCheckmateAnimation(inTurn);showEndModal(`Checkmate! ${inTurn === "white" ? "Black" : "White"} wins`)}else if(isStalemate(inTurn)){gameOver=!0;showEndModal("Stalemate! It's a draw")}}}
...
function pieceImgSrc(piece_name){const parts=piece_name.toLowerCase().split("_");const color=parts[0];const type=parts.slice(1).join("");return`Assets/images/pieces/${color}/${type}.png`}
