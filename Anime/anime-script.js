...
// Anikoto is only used to feed megaplay's /stream/s-2/ endpoint (the one with the
// best catalog coverage). It is NOT used for search/trending/details — AniList still
// owns all of that, which is also what keeps Scene Search (trace.moe) working, since
// trace.moe only ever returns AniList ids.
const ANIKOTO_API = 'https://anikotoapi.site';
// Static, same-origin file generated by tools/build-anikoto-map.js (see that script).
// Maps AniList id -> Anikoto catalog id. Loading it costs nothing against Anikoto's
// 60-req/120s rate limit, since it's just a regular file on your own server.
const ANIKOTO_MAP_URL = 'anikoto-map.json';
...
// --- ANIKOTO RESOLVER (feeds megaplay's /stream/s-2/ endpoint) ---
// Anikoto's API has no search-by-title or AniList-id lookup, only /recent-anime
// (paginated, ~880+ pages) and /series/{id}. So the AniList-id -> Anikoto-id
// crosswalk is built OFFLINE by tools/build-anikoto-map.js and shipped as a static
// anikoto-map.json. At runtime we just: look the AniList id up in that file, then
// make ONE live call to /series/{id} to get the episode's embed for the s-2 player.
// If anything is missing along the way, we return null and the caller falls back
// to the existing /stream/ani/{anilist-id} endpoint, so nothing ever breaks.
let anikotoMapPromise = null;
const anikotoEmbedCache = {};

function loadAnikotoMap() {
    if (!anikotoMapPromise) {
        anikotoMapPromise = fetch(ANIKOTO_MAP_URL)
            .then(res => res.ok ? res.json() : {})
            .catch(() => {
                console.warn(`Anikoto map ("${ANIKOTO_MAP_URL}") could not be loaded — MegaPlay HD will fall back to the standard MegaPlay server for everything. Run tools/build-anikoto-map.js to generate it.`);
                return {};
            });
    }
    return anikotoMapPromise;
}

// Resolves an iframe src via Anikoto + megaplay's s-2 endpoint, or null if there's
// no match / the lookup fails. NEVER throws — callers should always have a fallback.
async function resolveAnikotoSrc(anilistId, ep, lang) {
    const cacheKey = `${anilistId}-${ep}-${lang}`;
    if (cacheKey in anikotoEmbedCache) return anikotoEmbedCache[cacheKey];

    let result = null;
    try {
        const map = await loadAnikotoMap();
        const entry = map[anilistId];
        if (!entry) {
            anikotoEmbedCache[cacheKey] = null;
            return null;
        }

        const res = await fetch(`${ANIKOTO_API}/series/${entry.id}`);
        if (!res.ok) throw new Error(`/series/${entry.id} returned ${res.status}`);
        const json = await res.json();
        const episodes = json.episodes || (json.data && json.data.episodes) || [];

        const epData = episodes.find(e => {
            const num = e.episode_number ?? e.number ?? e.ep ?? e.episode;
            return Number(num) === Number(ep);
        });

        if (epData) {
            // Preferred: Anikoto hands back a ready-to-use embed URL.
            if (epData.embed_url && epData.embed_url[lang]) {
                result = epData.embed_url[lang];
            } else {
                // Fallback: build the s-2 URL ourselves from whatever id field is present.
                // NOTE: field name unconfirmed beyond docs text — check a live response
                // in devtools and adjust here if Anikoto uses a different key.
                const embedId = epData.episode_embed_id || epData.embed_id || epData.id;
                if (embedId) {
                    result = `https://megaplay.buzz/stream/s-2/${embedId}/${lang}`;
                }
            }
        }
    } catch (err) {
        console.warn('Anikoto resolution failed, falling back to standard MegaPlay:', err);
        result = null;
    }

    anikotoEmbedCache[cacheKey] = result;
    return result;
}
...
async function updatePlayer() {
    ...
    // Reflect the selection immediately — don't make button highlighting wait on a network call
    ...
    const btnServer3 = document.getElementById('btn-server3');
    if (btnServer3) btnServer3.classList.toggle('active', currentServer === 'anikoto');
    ...
    } else if (currentServer === 'anikoto') {
        // Snapshot the request so we can tell if the user moved on before it resolves
        const requestedAnimeId = currentAnimeId;
        const requestedEp = currentEp;
        const requestedAudio = audioMode;

        const resolved = await resolveAnikotoSrc(requestedAnimeId, requestedEp, requestedAudio);

        const stillCurrent = currentServer === 'anikoto' &&
            currentAnimeId === requestedAnimeId &&
            currentEp === requestedEp &&
            audioMode === requestedAudio;

        if (stillCurrent) {
            // Falls back to the standard AniList-id endpoint if Anikoto has no match
            iframe.src = resolved || `https://megaplay.buzz/stream/ani/${requestedAnimeId}/${requestedEp}/${requestedAudio}`;
        }
    }
}
...
// Handles typing/pasting an image URL: shows a live preview and clears any selected file
window.handleTraceMoeUrlInput = () => {
    ...
    // A pasted URL takes priority over a previously selected file
    fileInput.value = '';
    ...
    previewImg.onerror = () => {
        // Some hosts block hotlinking in the browser even though trace.moe can still fetch it server-side
        fileName.textContent = "Can't preview this URL, but we'll still try to search it";
    };
    previewImg.src = url;
